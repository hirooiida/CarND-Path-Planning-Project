...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
               &map_waypoints_dx,&map_waypoints_dy, &ref_vel, &lane, &ACC_VAL, &MAX_SPEED]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          bool is_close_ahead = false;
          bool is_close_left = false;
          bool is_close_right = false;
          ...
          if (is_close_ahead) {
            ...
            if (!is_close_left && lane > 0) {
              // Move to left
              lane -= 1;
            } else if (!is_close_right && lane < 2) {
              // Move to right
              lane += 1;
            } else {
              // Do not move and reduce speed
              ref_vel -= ACC_VAL;
            }

          } else {

            if ((lane == 0 && !is_close_right) || (lane == 2 && !is_close_left)) {
